...
const StrokeText = ({
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      // Compensate for the font's internal ascender whitespace so the viewBox perfectly snaps to the visual cap-height.
      const trimTop = Number(fontSize) * 0.20; // Approx 20% of font size is usually ascender whitespace above caps
      const trimLeft = Number(fontSize) * 0.02; // Small left trim to ensure the first glyph touches the edge perfectly

      const next = {
        x: bbox.x + trimLeft,
        y: bbox.y + trimTop,
        width: bbox.width - trimLeft,
        height: bbox.height - trimTop
      };
  ...
  }, [box, dash, drawDuration, fillDelay, delay, stagger, ease, trigger, fillMode, reverse]);
  ...
  return (
    ...
    >
      <svg className="stroke-text__svg" viewBox={viewBox} preserveAspectRatio="xMinYMin meet" aria-hidden="true">
        {fillMode === 'wipe' && box && (
          <defs>
